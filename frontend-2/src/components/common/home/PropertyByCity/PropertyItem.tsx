import React from "react";
import "./PropertyItem.scss";
interface Item {
    Photo : string,
    Name : string ,
    Number : number;
}
const PropertyItem = ({Photo,Name, Number}:Item) => {
  return (
    <div className="City-item relative cursor-pointer max-w-[300px] min-w-[300px] w-[350px] h-[450px] rounded-[25px] overflow-hidden">
      <div className="overlay w-full h-full bg-[rgba(0,0,0,0.34)] absolute z-[1]"></div>
      <img src={Photo} className="w-full object-cover h-full" alt="" />
      <h2 className="cityTitle text-[35px] font-bold absolute font-moul left-[30px] text-white top-[40px] z-[100]">
        {Name}
      </h2>
      <p className="number absolute top-[90px]  font-chenla left-[30px] text-[20px] z-[100] text-white ">
        <span className="font-bokor">{Number}</span> Property
      </p>
      <button
        type="button"
        className="flex SeeMore btnView justify-center gap-2 items-center mx-auto shadow-xl text-lg bg-gray-50 backdrop-blur-md lg:font-semibold isolation-auto border-gray-50 before:absolute before:w-full before:h-full  before:duration-700  before:left-[-100%]  before:rounded-full before:bg-black cursor-pointer text-black before:-z-10 before:aspect-square before:hover:scale-150 before:hover:duration-700 relative z-10 px-4 py-2 overflow-hidden border-2 rounded-full group"
      >
        See More
        <svg
          className="w-8 h-8 justify-end group-hover:rotate-90 group-hover:bg-gray-50 text-gray-50 ease-linear duration-300 rounded-full border border-gray-700 group-hover:border-none p-2 rotate-45"
          viewBox="0 0 16 19"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path
            d="M7 18C7 18.5523 7.44772 19 8 19C8.55228 19 9 18.5523 9 18H7ZM8.70711 0.292893C8.31658 -0.0976311 7.68342 -0.0976311 7.29289 0.292893L0.928932 6.65685C0.538408 7.04738 0.538408 7.68054 0.928932 8.07107C1.31946 8.46159 1.95262 8.46159 2.34315 8.07107L8 2.41421L13.6569 8.07107C14.0474 8.46159 14.6805 8.46159 15.0711 8.07107C15.4616 7.68054 15.4616 7.04738 15.0711 6.65685L8.70711 0.292893ZM9 18L9 1H7L7 18H9Z"
            className="fill-black group-hover:fill-black"
          ></path>
        </svg>
      </button>
    </div>
  );
};

export default PropertyItem;
